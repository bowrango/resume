...
\item Build graph compiler/transpiler using C++ to generate optimized assembly code for quantum circuits
\item Build client-side REST infrastructure for AWS and IBM cloud services to manage async jobs on quantum hardware
\item Develop sparse algorithms to compute expectation values of observables and decompose arbitrary unitary matrices
...
\item Developed presentations and examples for industry conferences (IBM QDC 22-25, AWS re: Invent 25)
\end{itemize}
...
\item Developed REST interface to cloud-based quantum annealing hardware; Evaluated binary optimization problems with hard/soft constraints to benchmark minor-embedding algorithms; Wrote parallel simulations to compute qubit transition probabilities for truncated Hamiltonian based on graph-theoretic formalism across coupling strength and energy separation parameters 
\end{itemize}}
...
\begin{itemize}[label=-]
\item Developed simulations of coupled oscillators in LTspice and modeled noisy phase dynamics using MATLAB
\item Developed real-time embedded C++ firmware using SPI and FreeRTOS to measure phases at 1MHz
\item Designed schematics and layouts for two multi-layer boards in Altium Designer
\item Soldered components and wrote MATLAB interfaces to test instruments for signal processing and Fourier analysis
\end{itemize}
\end{itemize}}
...
{\begin{itemize}
\item Wrote packet parser to process and analyze financial data using Python and Wireshark; Trained recurrent network to predict dash-camera misalignment using OpenCV and PyTorch; Built data pipeline and Bayesian network to predict trends for online video game using NetworkX and PyTorch
\end{itemize}} 
